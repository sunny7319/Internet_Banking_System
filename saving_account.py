...
class SavingAccount:
    ...
    def check_transactions(self, account_number):
        self.db_cursor.execute("SELECT * FROM saving WHERE account_number = ?", (account_number,))
    ...
    def deposit(self, account_number, amount):
        ...
        self.db_cursor.execute("INSERT INTO saving (account_number, transaction_type, amount) VALUES (?, ?, ?)",
                               (account_number, "자유적금", amount))
        self.db_connection.commit()
        print(f"적금 완료했습니다. 현재 잔액은 {new_balance} 원입니다.")
        print("--------------------")
    ...
    def withdraw(self, account_number):
        ...
        print(f"적금 해지 시 받는 금액은 {new_balance} 원입니다.")
        print("--------------------")
    ...
    def create_account(self, username, account_number, initial_balance, interest_rate, period):
        ...
        if user:
            self.db_cursor.execute("INSERT INTO users (username, account_number, balance, account_type, interest_rate, period) VALUES (?, ?, ?, ?, ?, ?)",
                               (username, account_number, initial_balance, "자유적금", interest_rate, period))
            self.db_cursor.execute("INSERT INTO saving (account_number, transaction_type, amount) VALUES (?, ?, ?)", (account_number, "신규가입", initial_balance))
            self.db_connection.commit()

        print("자유적금 계좌가 개설되었습니다.")
        print("--------------------")